...
## 협업에 관하여
- 이 과정을 통하여 좋은 머신러닝 모델을 만들 수 있는 실질적인 역량을 키우는 것이 목표였는데, 이 목표를 달성하기 위해서는 다른 그루들과 협업이 중요하다는 것을 알겠다. 나에게는 이게 가장 큰 도전일 듯. 

## 퀘스트를 통해서 Git, GitHub 사용법 숙지
- 퀘스트를 준비하면서 질문/답변을 하는 사이에 모르는 것을 알게 되고 이해도가 높아졌다. 생각보다 재밌는 듯.

## README.md를 편집하면서
- '오픈AI' 기사를 보면서 다음과 같은 생각이 들었다. 
> 사람은 경쟁에서 이기기 위해 ChatGPT 같은 인공지능을 사용하지 않을 수 없다. 그런데, 인공지능 서비스를 의존할 수록 사람의 생각과 행동이 인공지능에 학습되어 인공지능이 더 빠른 속도로 똑똑해질 것이다. 결국, 사람은 인공지능에 추월 당해서 도태 되지 않을까?
